...
struct amlvdec_h264_rpm {
	...
	uint16_t picture_structure;                   // 0x90
	...
	uint16_t prev_nal_unit_type;                  // 0xB9
...
#define VALIDATE_OFFSET(field, offset) \
    static_assert(offsetof(struct amlvdec_h264_rpm, field) == \
        (((offset) & ~3) + (3 - ((offset) & 3))) * sizeof(uint16_t), \
        "Offset mismatch for " #field)
...
#define IS_I_SLICE(slice_type) ((slice_type) == 2 || (slice_type) == 7)
#define IS_P_SLICE(slice_type) ((slice_type) == 0 || (slice_type) == 5)
#define IS_B_SLICE(slice_type) ((slice_type) == 1 || (slice_type) == 6)
...
static inline bool is_idr_pic(const struct amlvdec_h264_rpm *params) {
	return params->nal_unit_type == NAL_SLICE_IDR;
}
